...
class WP_Widget_ltg_adv_post_list extends WP_Widget {
	...
	function form( $instance ) {
		...
		<label><input type="radio" name="<?php echo $this->get_field_name( 'format' );  ?>" value="0" <?php if ( ! $instance['format'] ) { echo 'checked'; } ?> /><?php echo __( 'Thumbnail', LIGHTNING_ADVANCED_TEXTDOMAIN ) .'/'. __( 'Title', LIGHTNING_ADVANCED_TEXTDOMAIN ) .'/'. __( 'Date', LIGHTNING_ADVANCED_TEXTDOMAIN ); ?></label><br/>
		<label><input type="radio" name="<?php echo $this->get_field_name( 'format' );  ?>" value="1" <?php if ( $instance['format'] == 1 ) { echo 'checked'; } ?>/><?php echo __( 'Date', LIGHTNING_ADVANCED_TEXTDOMAIN ) .'/'. __( 'Category', LIGHTNING_ADVANCED_TEXTDOMAIN ) .'/'. __( 'Title', LIGHTNING_ADVANCED_TEXTDOMAIN ); ?></label>
        <br/><br/>

		<?php echo _e( 'Order by', LIGHTNING_ADVANCED_TEXTDOMAIN ); ?>:<br/>
		<label style="padding-bottom: 0.5em"><input type="radio" name="<?php echo $this->get_field_name( 'orderby' );  ?>" value="date" <?php if ( $instance['orderby'] != 'modified' ) { echo 'checked'; } ?> /><?php _e( 'Publish date', LIGHTNING_ADVANCED_TEXTDOMAIN ); ?></label><br/>
		<label><input type="radio" name="<?php echo $this->get_field_name( 'orderby' );  ?>" value="modified" <?php if ( $instance['orderby'] == 'modified' ) { echo 'checked'; } ?>/><?php _e( 'Modified date', LIGHTNING_ADVANCED_TEXTDOMAIN ); ?></label>
        <br/><br/>
